...
$(document).ready(function(){
	$('.mobile-drawer-header').click(function() {
		
		$(this).next('.mobile-drawer').toggle('fast');
	...
	if($(window).width() < 650) {
		...
		$('html').addClass('mobile-all'); 
	} else {
		$('#content').css('display', 'inherit');
		$('.mobile').css('display', 'none');
		$('html').removeClass('mobile-all'); 
	}
...
$(window).on('resize',function() {
	...
	/*if($(window).width() > 650) {
		var leftPos = $(window).width()-1533;
		if(leftPos < 0) { leftPos = 0;} 
		if(leftPos > 117.0) {leftPos = 117.0;}
		var leftPosPerc = leftPos/117.0;
		var leftPosNewPos = (1.0-leftPosPerc)*(-43);
		$('#logo').css('left', leftPosNewPos+"px"); 
	
		// 1530, 1150
		leftPos = $(window).width()-1350;
		if(leftPos < 0) { leftPos = 0;} 
		if(leftPos > 250.0) {leftPos = 250.0;}
		leftPosPerc = leftPos/250.0;
		var newHeight = (leftPosPerc)*(125) + 84;  // between  210 to 85
		var newBackgroundPos = 15.0*(1-leftPosPerc);
		leftPosNewPos = (leftPosPerc)*(-43);
		if($(window).width() < 1533) {$('#logo').css('left', leftPosNewPos+"px"); }
		//$('#logo').css('height', newHeight+"px");  
		$('#logo').css('background-position-y', -newBackgroundPos+"px"); 
  	}*/
}
